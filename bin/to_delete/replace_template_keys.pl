...
if (exists $options{'iterator_list'}) {
    ...
    if ( $options{output_xml} =~ /\.gz$/ ) {
        open (FILE, "| /bin/gzip -c > $options{output_xml}" ) or $logger->logdie("Can't open file $options{'output_xml'}");
...
sub replacekeys{
    ...
    if ( $outputfile =~ /\.gz$/ ) {
	open (OUTPUTFILE, "| /bin/gzip -c > $outputfile" ) or $logger->logdie("Could not open output template file $outputfile: $!");
