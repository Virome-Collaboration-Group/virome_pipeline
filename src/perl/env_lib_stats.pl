    ...
    foreach my $category (@$lib_array) {
       ...
       #loop through all categories/types i.e: eco system, sphere, lib_type etc.
        foreach my $feature ( keys %$category ) {
            ...
            open( OUT, ">$xFile" )
              or $logger->logdie("Cannot open file $xFile to write");
            open( IDOUT, ">$idFile" )
              or $logger->logdie("Cannot open file $idFile to write");
...
sub bestEvalue {
    ...
    {
        my $blast = $dbh->prepare(
qq{SELECT m.$field as field, m.lib_prefix, min(b.e_value) as eval, m.lib_shortname
                                   FROM blastp b
                                   INNER JOIN
